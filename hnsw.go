...
import (
	...
	"github.com/kuentra-official/hnsw/modules"
...
func (this *Hnsw) BytesSize() uint64 {
	...
	for i := 1; i < maxLevel; i++ {
		pointersSize += (float64(this.config.mMax*HNSW_VERTEX_EDGE_BYTES) + mutb) * xMath.Exp(float64(i)/-float64(this.config.levelMultiplier))
	}

	verticesDataSize := atomic.LoadUint64(&this.bytesSize)
	return uint64(xMath.Floor(float64(this.Len())*pointersSize)) + verticesDataSize
}

func (this *Hnsw) Insert(id uuid.UUID, value mathk.Vector, metadata Metadata, vertexLevel int) error {
	...
	for l := mathk.MinInt(entrypoint.level, vertex.level); l >= 0; l-- {
...
func (this *Hnsw) Get(id uuid.UUID) (mathk.Vector, error) {
...
func (this *Hnsw) Search(ctx context.Context, query mathk.Vector, k uint) (SearchResult, error) {
	entrypoint := (*hnswVertex)(atomic.LoadPointer(&this.entrypoint))
	if entrypoint == nil {
	...
	ef := mathk.MaxInt(this.config.ef, int(k))
	...
	n := mathk.MinInt(int(k), neighbors.Len())
...
func (this *Hnsw) RandomLevel() int {
	return mathk.Floor(mathk.RandomExponential(this.config.levelMultiplier))
}

func (this *Hnsw) getVerticesShard(id uuid.UUID) (map[uuid.UUID]*hnswVertex, *sync.RWMutex) {
	shardIdx := modules.UuidMod(id, uint64(VERTICES_MAP_SHARD_COUNT))
...
func (this *Hnsw) greedyClosestNeighbor(query mathk.Vector, entrypoint *hnswVertex, minDistance float32, level int) (*hnswVertex, float32) {
	for {
...
func (this *Hnsw) searchLevel(query mathk.Vector, entrypoint *hnswVertex, ef, level int) modules.PriorityQueue {
	entrypointDistance := this.space.Distance(query, entrypoint.vector)

	pqItem := modules.NewPriorityQueueItem(entrypointDistance, entrypoint)
	candidateVertices := modules.NewMinPriorityQueue(pqItem)
	resultVertices := modules.NewMaxPriorityQueue(pqItem)
	...
	for candidateVertices.Len() > 0 {
		...
		for neighbor, _ := range candidate.edges[level] {
			...
			if (distance < lowerBound) || (resultVertices.Len() < ef) {
				pqItem := modules.NewPriorityQueueItem(distance, neighbor)
...
func (this *Hnsw) selectNeighbors(neighbors modules.PriorityQueue, k int) modules.PriorityQueue {
	for neighbors.Len() > k {
		neighbors.Pop()
	}
...
func (this *Hnsw) selectNeighborsHeuristic(query mathk.Vector, neighbors modules.PriorityQueue, k, level int, extendCandidates, keepPruned bool) modules.PriorityQueue {
	...
	if extendCandidates {
		for neighbors.Len() > 0 {
			...
			for neighbor, _ := range candidate.edges[level] {
				...
				distance := this.space.Distance(query, neighbor.vector)
				candidateVertices.Push(modules.NewPriorityQueueItem(distance, neighbor))
			}
			candidate.edgeMutexes[level].RUnlock()
		}
	}

	result := modules.NewMaxPriorityQueue()
...
func (this *Hnsw) pruneNeighbors(vertex *hnswVertex, k, level int) {
	neighborsQueue := modules.NewMaxPriorityQueue()
	...
	for neighbor, distance := range vertex.edges[level] {
		...
		neighborsQueue.Push(modules.NewPriorityQueueItem(distance, neighbor))
	}
